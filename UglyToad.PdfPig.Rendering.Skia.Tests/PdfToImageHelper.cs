...
{
    private const string ErrorFolder = "ErrorImages";
    private const byte Threshold = 2;
    ...
    private const double MaxDifferingPixelRatio = 0.001;
    ...
    private static string ResolveExpectedImagePath(string expectedFile)
    {
        ...
        if (os is not null)
        {
            string platformSpecific = Path.Combine(Helper.ExpectedImagesFolder, os, expectedFile);
        ...
        return Path.Combine(Helper.ExpectedImagesFolder, expectedFile);
    }
    ...
    private static SKBitmap CreateEmptyDiffImage(int minWidth, int minHeight, int maxWidth, int maxHeight)
    {
