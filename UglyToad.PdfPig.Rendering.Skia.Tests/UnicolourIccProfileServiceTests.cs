...
public class UnicolourIccProfileServiceTests
{
    ...
    public void TryGetProfile_LoadsRealSrgbProfile()
    {
        ...
        bool ok = UnicolourIccProfileService.Instance.TryGetProfile(
            bytes.AsMemory(), out var profile);
    ...
    public void TryGetProfile_DoesNotMutateInputBytes()
    {
        ...
        _ = UnicolourIccProfileService.Instance.TryGetProfile(
            bytes.AsMemory(), out _);
    ...
    public void ToRgb_ClipsOutOfGamutComponentsToZeroOrOne()
    {
        ...
        byte[] bytes = LoadEmbeddedSrgbProfile();
        Assert.True(UnicolourIccProfileService.Instance.TryGetProfile(
            bytes.AsMemory(), out var profile));
    ...
    [Fact]
    public void TryGetProfile_DerivesComponentCountFromTheProfileNotTheCaller()
    {
        // sRGB2014 is a 3-channel 'RGB ' profile. The count is no longer supplied by the caller, so it
        // cannot be wrong: before this, passing 4 produced a profile that cheerfully reported 4 and made
        // the /N-vs-profile guard in ICCBasedColorSpaceDetails unfalsifiable.
        byte[] bytes = LoadEmbeddedSrgbProfile();

        Assert.True(UnicolourIccProfileService.Instance.TryGetProfile(bytes.AsMemory(), out var profile));
        Assert.Equal(3, profile!.NumberOfComponents);

        // 'RGB ' is not L*a*b*, so the profile reports the ordinary [0, 1] encoding for all three channels -
        // which is how ICCBasedColorSpaceDetails.GetProfileRanges knows to leave the colour space's own
        // /Range in charge. (This asserted profile.IsLabInput before ComponentRanges replaced it on
        // IIccProfile; the ranges are the same fact, stated in the form the core actually consumes.)
        Assert.Equal(new double[] { 0.0, 1.0, 0.0, 1.0, 0.0, 1.0 }, profile.ComponentRanges);
    }
}
