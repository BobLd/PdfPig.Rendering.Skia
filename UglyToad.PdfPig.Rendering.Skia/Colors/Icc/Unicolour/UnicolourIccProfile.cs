...
{
    /// <summary>
    /// A parsed Unicolour ICC profile. Owns a per-intent
    /// <see cref="IIccTransform"/> cache so each intent is configured at
    /// most once. Safe for concurrent reads.
    /// </summary>
    internal sealed class UnicolourIccProfile : IIccProfile
    {
        private readonly Profile profile;
        private readonly ConcurrentDictionary<RenderingIntent, IIccTransform> transforms = new();
        ...
        {
            transform = transforms.GetOrAdd(intent, BuildFor);
        ...
        private IIccTransform BuildFor(RenderingIntent intent)
        {
            ...
            return new UnicolourIccTransformCached(config, NumberOfComponents);
        }
