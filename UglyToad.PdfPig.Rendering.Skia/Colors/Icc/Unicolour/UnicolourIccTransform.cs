...
namespace UglyToad.PdfPig.Colors.Icc.Unicolour
{
    ...
    /// <see cref="Wacton.Unicolour.Configuration"/>.
    /// </summary>
    internal sealed class UnicolourIccTransform : IIccTransform
    {
        ...
        public UnicolourIccTransform(Configuration config, int numberOfComponents)
        {
            this.config = config;
    ...
    internal sealed class UnicolourIccTransformCached : IIccTransform
    {
        ...
        public UnicolourIccTransformCached(Configuration config, int numberOfComponents)
        {
            inner = new UnicolourIccTransform(config, numberOfComponents);
        }

        public (double r, double g, double b) ToRgb(ReadOnlySpan<double> values)
        {
            var key = ColorCacheKey.FromDoubles(values);
        ...
        public void Transform(ReadOnlySpan<byte> src, Span<byte> dstRgb)
        {
            ...
            for (int p = 0; p < pixels; p++)
            {
                ...
                if (!cache.TryGetValue(key, out var rgb))
                {
                    ...
                    var computed = inner.ToRgb(buffer);
                    rgb = new Rgb24(
